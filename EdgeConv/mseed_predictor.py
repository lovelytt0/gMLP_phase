...
# from gMLPhase.EqT_utils import f1, SeqSelfAttention, FeedForward, LayerNormalization
...
#                   metrics = [f1])
    ...
    pre_model = gMLPmodel.load_from_checkpoint(checkpoint_path='/home/tian_feng/UCLA/gMLP_phase/gMLP_phase/test_trainer/test15/checkpoints/last.ckpt',hparams_file='/home/tian_feng/UCLA/gMLP_phase/gMLP_phase/test_trainer/test15/default/version_0/hparams.yaml')
...
#             print(meta, time_slots, comp_types, data_set)
            pred_generator = PreLoadGeneratorTest(meta["trace_start_time"], data_set, **params_pred)
            
            X = pred_generator.__getitem__(0)
#             predD, predP, predS = model.predict_generator(pred_generator)
            X = torch.from_numpy(X['input']).float().transpose_(1, 2)
            
            output = sig(pre_model.forward(X)).detach().numpy()
#             print(output.shape)
            predD =output[:,0,:]
            predP =output[:,1,:]
            predS =output[:,2,:]
            ...
            for ix in range(len(predD)):
                matches, pick_errors, yh3 =  _picker(args, predD[ix][:], predP[ix][:], predS[ix][:])        
                if (len(matches) >= 1) and ((matches[list(matches)[0]][3] or matches[list(matches)[0]][6])):
                    ...
                    if plt_n < args['number_of_plots'] and post_write > pre_write:
                        _plotter_prediction(data_set[meta["trace_start_time"][ix]], args, save_figs, predD[ix][:], predP[ix][:], predS[ix][:], meta["trace_start_time"][ix], matches)
                        plt_n += 1            
