...
from EdgeConv.trainer import Graphmodel
...
#     save_figs2 = os.path.join(save_dir, 'figures2')

    if os.path.isdir(save_figs):
        shutil.rmtree(save_figs)  
    os.makedirs(save_figs) 
#     if os.path.isdir(save_figs2):
#         shutil.rmtree(save_figs2)  
#     os.makedirs(save_figs2) 
    test = np.load(os.path.join(args['output_name'],'validation.npy'),allow_pickle=True)
    ...
    test_generator = DataGeneratorGNN(list_IDs=test,**params_validation) 
        
    
    pre_model = gMLPmodel.load_from_checkpoint(checkpoint_path='/home/tian_feng/UCLA/gMLP_phase/gMLP_phase/test_trainer/test12/checkpoints/epoch=41-step=336419.ckpt',hparams_file='/home/tian_feng/UCLA/gMLP_phase/gMLP_phase/test_trainer/test12/default/version_0/hparams.yaml')
    
    pre_model.eval() 

    
    model = Graphmodel(pre_model).load_from_checkpoint(checkpoint_path=os.path.join(args['output_name'],args['input_model']),pre_model=pre_model)
    ...
    for idx in range(10):
        ...
        folder = os.path.join(save_figs, 'event_'+str(idx),'single')
        folder2 = os.path.join(save_figs, 'event_'+str(idx),'multi')
        ...
        os.makedirs(folder2)     
        X = test_generator.get_full_item(idx)
        spt_ls = X[3]
        sst_ls = X[4]
        evi_ls = X[5] 

        tmp =  sig(pre_model.forward(X[0])).detach().numpy()
        pred_DD_mean = tmp[:,0,:]
        pred_PP_mean = tmp[:,1,:]
        pred_SS_mean = tmp[:,2,:]
        ...
        tmp =  sig(model.forward(X)).detach().numpy()
        pred_DD_mean = tmp[:,0,:]
        pred_PP_mean = tmp[:,1,:]
        pred_SS_mean = tmp[:,2,:]
    ...
    for _ in range(int(np.ceil(len(test) / args['batch_size']))):
        ...
        X = np.zeros(( len(new_list), 3, args['input_dimention'][0]))
        ID = new_list[0]
        test_set={}
        for nb, ID in enumerate(new_list):
            try:
                if ID.split('_')[-1] == 'EV':
                ...
                data = _normalize(data, args['normalization_mode'])                          
...
#                 raise
        ...
        tmp =  sig(model.forward(X)).detach().numpy()# model.predict_generator(generator=test_generator)
        pred_DD_mean = tmp[:,0,:]
        pred_PP_mean = tmp[:,1,:]
        pred_SS_mean = tmp[:,2,:]
...
#     (input_hdf5='/home/tian_feng/UCLA/merge.hdf5',
#         input_trainset= 'multitraces.npy',
#         output_name='GNN/test1',      
#         input_model='checkpoints/epoch=25-step=166451.ckpt',
#         hparams_file = 'default/version_0/hparams.yaml', 
#         model_folder='/home/tian_feng/UCLA/gMLP_phase/gMLP_phase/test_trainer/test11',
    
    
tester(input_hdf5='/home/tian_feng/UCLA/merge.hdf5',
       input_model='checkpoints/epoch=0-step=45224.ckpt',
       hparams_file = 'default/version_0/hparams.yaml', output_name='/home/tian_feng/UCLA/gMLP_phase/gMLP_phase/GNN/test2',
